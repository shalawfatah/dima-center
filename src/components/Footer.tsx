...
export default function Footer({
  ...
  // Get fonts from generalSettings
  ...
  // Create dynamic @font-face rules if fonts exist
  let dynamicFontFaceCSS = ''

  if (typography) {
    // Get the locale-specific font group
    const localeMap = {
      ckb: 'kurdish',
      ar: 'arabic',
      en: 'english',
    ...
    if (headingFontObj && typeof headingFontObj === 'object' && headingFontObj.url) {
      const fontName = `FooterHeading_${currentLocale}`
    ...
    if (bodyFontObj && typeof bodyFontObj === 'object' && bodyFontObj.url) {
      const fontName = `FooterBody_${currentLocale}`
  ...
  return (
    <>
      {/* Inject dynamic font-face rules */}
        ...
        style={
          {
            ...
            '--footer-body-color': bodyColor,
            '--footer-border-color': borderColor,
            '--footer-heading-font': headingFont,
            '--footer-body-font': bodyFont,
      ...
      >
        <div className={styles.footerGrid}>
          {/* About Segment */}
          <div>
            <h4 className={styles.heading} style={{ fontFamily: 'var(--footer-heading-font)' }}>
              {t.aboutTitle}
            </h4>
          ...
          <div>
            ...
            <p className={styles.addressLine1}>{t.addressLine1}</p>
            <p
              className={`${styles.addressLine2} ${
                isRtl ? styles.addressLine2Rtl : styles.addressLine2Ltr
              }`}
            >
              {t.addressLine2}
            </p>
            ...
            <p className={styles.phone}>
              ...
              <span dir="ltr" className={styles.phoneValue}>
                {t.phoneValue}
              </span>
            </p>
          </div>
          ...
          <div>
            ...
            <p className={styles.policyDesc}>{t.policyDesc}</p>
          </div>
        </div>

        {/* Bottom Bar Elements */}
        <div className={styles.bottomBar} style={{ fontFamily: 'var(--footer-body-font)' }}>
